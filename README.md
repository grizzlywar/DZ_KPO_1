...
- **Доменные модели:**  
  Классы `Animal`, `Herbo`, `Predator` и их конкретные реализации (например, `Monkey`, `Rabbit`, `Tiger`, `Wolf`) отвечают только за хранение данных и базовую логику, связанную с животными.
- **Сервисы:**  
  Класс `Zoo` управляет коллекциями животных и инвентаризацией, а класс `VetClinic` отвечает за проверку состояния здоровья животных.
- **Бизнес-логика:**  
  Класс `ZooManager` инкапсулирует логику добавления животных и инвентарных объектов, делегируя конкретные операции соответствующим сервисам.
- **Пользовательский интерфейс:**  
  Класс `UserInterface` отвечает за взаимодействие с пользователем через консоль.

### Open/Closed Principle (OCP)
Для расширения функциональности (например, добавления новых типов животных или инвентарных объектов) достаточно создать новый класс, унаследованный от базового (`Animal` или `Thing`), без изменения существующего кода.

### Liskov Substitution Principle (LSP)
Все конкретные реализации животных (например, `Rabbit`, `Tiger`, `Monkey`) можно использовать через базовый класс `Animal`, так как они корректно реализуют общие методы (например, `IsSuitableForContactZoo`).

### Interface Segregation Principle (ISP)
Интерфейсы `IAlive` и `IInventory` содержат только необходимые свойства для животных и инвентарных объектов, что позволяет реализовывать их без ненужной нагрузки.

### Dependency Inversion Principle (DIP)
Высокоуровневые модули (например, `ZooManager` и `UserInterface`) зависят от абстракций (интерфейсы `IZoo` и `IVetClinic`), а не от конкретных реализаций. Это позволяет легко заменять сервисы при тестировании или расширении функциональности.
...
**Что происходит:**
Программа ищет животное по номеру. Если оно найдено и соответствует критериям (например, травоядное с достаточной добротой(больше или равно 5)), оно добавляется в специальную коллекцию контактного зоопарка. Если животное не найдено или не соответствует критериям, выводится сообщение.
